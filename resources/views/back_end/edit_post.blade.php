@extends('back_end.layout')
        @section('content')
            <div class="box-content">
              <form class="form-horizontal" action="{{url('/admin/updatepost/'.$post->id)}}" method="post" enctype = "multipart/form-data">
              {{csrf_field()}}
						  <fieldset>
							<div class="control-group">
							  <label class="control-label" for="typeahead">Post Title</label>
							  <div class="controls">
								<input type="text" class="span6 typeahead" id="typeahead"  name = "post_title"value = {{$post->title}}>
							  </div>
              </div>             
              <div class="control-group">
							  <label class="control-label" for="typeahead">Choose Tag</label>
							  <div class="controls">
								<input type="text" class="span6 typeahead" id="typeahead"  name = "choose_tag"value = {{$post->tag}}>
							  </div>
							</div>
							<div class="control-group">
							...
							<div class="control-group hidden-phone">
							  ...
							  <div class="controls">
								<textarea class="cleditor" id="textarea2" rows="3" name = "post_desc">{{$post->article}}</textarea>
							  </div>
							</div>
